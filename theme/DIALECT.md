...
built by `build.py` into one self-contained HTML file that
opens from `file://` with the network off. The shell (topbar, sidebar,
router, pager, tests) is inherited, never rebuilt.

To start a new workshop: make a top-level folder holding a `workshop.md`
conforming to this dialect, with its figures in `images/` beside it. Nothing
registers it — the build discovers any folder shaped that way.

`theme/assets/tokens.css` holds every color, and each value there is a
Commvault brand color or a permitted tint or shade of one, with the reasoning
beside it. Change nothing there without a brand reason.

```
python3 build.py                  # every workshop -> _site/<folder>/index.html
python3 build.py <folder>         # one of them; the index still lists all
python3 build.py --mode room      # SOLO chapters as inherited stubs
```

`index.html` is not a preference: GitHub Pages serves a directory by its
index, and so does `python -m http.server`.

`<your-codename>` stays literal in a published page, and that is correct —
a reader is looking at the shape of the command, not at their own run. The
guide served inside a participant's own environment is the one that
substitutes it.
...
drift. In v1 a test pins that no checkpoint is lost in the swap; it was not
carried into this repository, where only the solo build ships.
...
``` starting ✓✗⏱   the diagnostic rows: ✓ expected output, ✗ what to do
                   ...
                   the text. The glyph counts only in the MARKER COLUMN,
                   at most two spaces in: indented output may open with
                   ✓ or ✗ and stays output, which is what lets a box
                   quote `✓ recovery proven` and `✗ Detect ...` as the
                   engine prints them. Getting any of this wrong is a
                   build error with the line number, never a silently
                   wrong page
...
@icon-name         an optional prefix on a `list` row: attaches an official
                   Commvault icon (theme/assets/icons/). Explicit in the markdown
...
section and the one-next-action prompt. Scope honesty lives in README.md
and STATUS.md; adoption steps live in each chapter's NEXT row. Do not
...
The figure SVGs are GENERATED by the workshop's own `figures/make_*.py`,
which mirror their colors from `theme/assets/tokens.css` and embed the official
...
```
for f in <workshop>/figures/make_*.py; do python3 "$f"; done
```

Icons live in `theme/assets/icons/` and are official Commvault icons only, in the
...
An unknown `@name` fails the build. An icon no workshop uses is not warned
about: with several workshops in the repository, one workshop's unused icon
is another's.
...
The stock phrases, canonical. These are the sentences the first workshop
...
## 10. What the build does NOT check, and in this repository nothing does

Two editorial rules hold over everything this repo ships, including your
markdown. The BUILD does not enforce them, so a page with either builds
happily:

```
no em dashes        Use a spaced en dash where a sentence breaks, or a
                    period, a comma, or a rephrase. The em dash is not in
                    the Commvault editorial guide at all. Quoted command
                    output keeps whatever the command prints
US spelling         The guide defers to AP Style. color, center, analyze,
                    catalog, program, license, labeled, authorize
```

In v1 (`resops-cvlt-azure`) both are enforced by
`tests/test_participant_guide.py`, as patterns over the suffix classes that
separate British from US spelling rather than word lists. That guard was not
carried across when v2 forked, so HERE THEY ARE RULES WITHOUT A GUARD: a
reviewer checks them by eye, and they have slipped before. Porting the guard
is listed in STATUS.md.
