...
def diag_rows(body, body_start):
    ...
    A row's glyph sits in the marker column, at most two spaces in. Anything
    further right is expected OUTPUT, even when it opens with a glyph — and it
    routinely does, because the engine prints `✓ recovery proven` and
    `✗ Detect ...` as its own lines. Matching on the glyph alone made those
    unquotable: the box could not show what the command actually prints.
    """
    rows = []
    for idx, line in enumerate(body):
        s = line.strip()
        in_marker_column = len(line) - len(line.lstrip()) <= 2
        if s and s[0] in GLYPHS and in_marker_column:
            rest = s[1:].strip()
...
def strip_rows(body, body_start):
    ...
    A LABEL MAY BE MORE THAN ONE WORD. It was `[A-Z]+`, so `DONE WHEN` did not
    match and was silently glued onto the row above it as continuation text --
    no error, just a strip row that had swallowed the next one. diag_rows has
    allowed multi-word labels all along, so this was an inconsistency between
    two halves of the same dialect rather than a rule. Non-greedy, because the
    separator is the first run of two or more spaces."""
    rows = []
    for idx, line in enumerate(body):
        s = line.strip()
        m = re.match(r"^([A-Z][A-Z ]*?)\s{2,}(.*)$", s) if s else None
        if m:
